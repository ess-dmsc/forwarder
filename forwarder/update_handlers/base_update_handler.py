...
class BaseUpdateHandler:
    def __init__(self, producer: KafkaProducer, pv_name: str, output_topic: str):
        ...
        self._last_timestamp = datetime(year=1900, month=1, day=1, hour=0, minute=0, second=0, tzinfo=timezone.utc)
    ...
    def _publish_message(self, message: bytes, timestamp_ns: int):
        ...
        if message_datetime < self._last_timestamp:
            self._logger.error(f"Rejecting update from PV \"{self._pv_name}\" as its timestamp is older than the previous message timestamp from that PV.")
            return
