...
from streaming_data_types.fbschemas.pv_alarm_state_pvAl.AlarmSeverity import AlarmSeverity
...
def _extract_alarm_info(update: p4p.Value) -> Tuple[AlarmState, AlarmSeverity, str]:
    try:
        try:
            ...
            return AlarmState.UNDEFINED, AlarmSeverity.UNDEFINED, "Unknown alarm and/or severity."
    except AttributeError:
        return AlarmState.UNDEFINED, AlarmSeverity.UNDEFINED, "No alarm (or missing) info in PV update"
...
class pvAl_Serialiser:
    ...
    def _serialise(self, timestamp, alarm: AlarmState, severity: AlarmSeverity, ca_alarm: CAAlarmState = CAAlarmState.UDF, message="") -> Tuple[bytes, int]:
    ...
    def _serialise_undefined(self) -> Tuple[bytes, int]:
        return self._serialise(datetime.now(tz=timezone.utc), alarm=AlarmState.UNDEFINED, ca_alarm=CAAlarmState.UDF, severity=AlarmSeverity.UNDEFINED)
    ...
    def pva_serialise(
        ...
        timestamp = datetime.fromtimestamp(update.timeStamp.secondsPastEpoch, tz=timezone.utc) + timedelta(microseconds=update.timeStamp.nanoseconds/1000)
        return self._serialise(alarm=alarm, severity=severity, timestamp=timestamp, ca_alarm=CAAlarmState.UDF, message=msg)
    ...
    def ca_serialise(
        ...
        try:
            ...
            return self._serialise(alarm=AlarmState.UNDEFINED, severity=AlarmSeverity.UNDEFINED, ca_alarm=CAAlarmState.UDF, timestamp=timestamp)
        return self._serialise(alarm=alarm, severity=severity, ca_alarm=ca_alarm, timestamp=timestamp)
