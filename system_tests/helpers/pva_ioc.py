...
DOUBLE3 = SharedPV(nt=NTScalar('d'), initial={"value": 0.0,
"timeStamp.secondsPastEpoch": int(time.time()), "timeStamp.nanoseconds": 0,
                                              "alarm.message": "HIGH_ALARM",
                                                               "alarm.severity": 1,
                                                               "alarm.status": 4,}, )
STR = SharedPV(nt=NTScalar('s'), initial="Initial string")
ENUM = SharedPV(nt=NTEnum(), initial={"index":0, "choices": ["Yes", "No", "Maybe", "Perhaps"]})
...
@FLOATARRAY.put
def handle(pv, op):
    pv.post({"value": op.value(), "timeStamp.secondsPastEpoch": int(time.time()), "timeStamp.nanoseconds": 0})
    op.done()
